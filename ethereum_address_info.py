...
class EthereumAddressInfo:
    ...
    def _make_request(self, params: Dict[str, str]) -> Union[str, List[Dict[str, Any]]]:
        ...
        for attempt in range(1, self.MAX_RETRIES + 1):
            try:
                ...
                raise EthereumAPIError(f"Request exception occurred: {e}")
    ...
    def _convert_wei_to_eth(wei: Union[str, int], decimals: int = 18) -> float:
        """Convert Wei to Ether with configurable decimals (for tokens)."""
        try:
    ...
    def get_balance(self) -> float:
        """Get ETH balance of the address."""
        params = {
            "module": "account",
            "action": "balance",
    ...
    def get_token_balance(self, contract_address: str, decimals: int = 18) -> Optional[float]:
        """Get ERC-20 token balance. Requires contract address and optional decimals."""
        ...
        logger.info(f"Token Balance: {balance:.6f}")
        return balance
...
def main():
    ...
    if not api_key or not address:
        logger.error("Missing ETHERSCAN_API_KEY or ETHEREUM_ADDRESS environment variables.")
        return
    ...
    try:
        with EthereumAddressInfo(api_key, address) as eth:
            ...
            if contract_address:
                eth.get_token_balance(contract_address, decimals=token_decimals)
